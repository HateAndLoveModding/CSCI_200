...
class Cube {
     ...
     * 
     */
    void isCornerCorrect(int&, std::string&);

    /**
     * @brief Rotates the corners in the last layer to the correct position
     * 
     */
    void lastLayerCornerHelper(int&, std::string&);
    ...
    /**
     * @brief Checks if a corner that has white on it is in the buffer position. If not, it turns the yellow layer clockwise, if yes, then it inserts the corner into the correct position.
     * 
     */
    void cornerHelper();
    ...
    /**
     * @brief Determines if the cube is solved. True if solved, else false.
     * 
     * @return true 
     * @return false 
     */
